...
def _difference_combats(
    ...
    for i, combat in combats.iterrows():
        ...
        for nom in caracteristiques["NAME"].values:
            if fuzz.ratio(nom.lower(), combattant_1.lower()) > 95:
                stats_combattant_1 = caracteristiques[
                    caracteristiques["NAME"].str.lower() == nom.lower()
                ].iloc[0]
                break
        ...
        for column in numeric_columns:
            ...
            if isinstance(stats_combattant_1[column], (int, float)) and isinstance(
                stats_combattant_2[column], (int, float)
            ):
                combats.loc[i, f"diff_{column}"] = (
                    stats_combattant_1[column] - stats_combattant_2[column]
                )
...
def _transformation_debut_octogone(data):
    ...
    data["DÉBUT DE L'OCTOGONE"] = (
        pd.to_datetime("today") - data["DÉBUT DE L'OCTOGONE"]
    ).dt.days // 12
    return data
