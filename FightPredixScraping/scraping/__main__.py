...
from .lib_front_page_ufc import _page_principal_UFC
...
def main():
    ...
    Data.to_json("Data/Data_ufc_fighters.json", orient="columns", index=False)
    main_driver.quit()
    ...
    Data = _main_tapology()
    Data.to_pandas().to_json("Data/Data_ufc_complet.json", orient="columns", index=False)
    ...
    combats.to_json("Data/Data_ufc_combats_simple.json", orient="columns", index=False)
    main_driver.quit()

    combats = pd.read_json("Data/Data_ufc_combats_simple.json")
    main_driver = webdriver.Chrome(options=chrome_options)
    ...
    combats.to_json("Data/Data_ufc_combats_complet.json", orient="columns", index=False)
    main_driver.quit()

    logger.info("Suppression des fichiers temporaires")
    for file_path in [
        "Data/Data_ufc_fighters.json",
        "Data/Data_jointes_ufc_tapology.json",
        "Data/final_tapology.json"
        "Data/data_tapology.json",
        "Data/Data_ufc_combats_simple.json",
        "Data/clean_tapology.json",
    ]:
