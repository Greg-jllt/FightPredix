...
def _prediction_streamlit(
    ...
    for num1, num2 in zip((indice_nom1, indice_nom2), (indice_nom2, indice_nom1)):
        ...
        type_mapping = {
            col: "number" if col in var_num_en_plus + var_moyenne else "object"
            for col in combat.columns
        }
    ...
    results = (
        (predictions[1].flatten()[1] + predictions[0].flatten()[0]) / 2,
        (predictions[1].flatten()[0] + predictions[0].flatten()[1]) / 2,
    )
