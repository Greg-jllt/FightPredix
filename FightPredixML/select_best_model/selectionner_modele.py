...
class ModelSelectionError(Exception):
    """Exception levée lorsque la sélection du modèle échoue."""
...
def _comparer_score_entrainement(
    modeles: list[dict[str | Pipeline | float]],
) -> Pipeline:
    """
...
def _tester_surapprentissage(
    X_test: pd.DataFrame, y_test: pd.DataFrame, modele: dict[str, Pipeline, float]
) -> bool:
    """
    ...
    if diff_score_entrainement_test > 0.05:
        Console().print(
...
def _selectionner_meilleurs_modeles(
    X_test: pd.DataFrame, y_test: pd.Series, modeles: list[dict[str, Pipeline | float]]
) -> Pipeline | None:
    """
    ...
    for modele in modeles:
        if _tester_surapprentissage(X_test, y_test, modele):
            liste_modeles.remove(modele)
    if len(liste_modeles) == 0:
        raise ModelSelectionError(
            "Aucun modèle n'a été sélectionné car tous surapprennent"
        )
